			...
			for j in range(0,cols):
				...
				for k in range(xmin,xmax):
					for l in range(ymin,ymax):
						s+=input[k*cols+l] # changed "rows" to "cols" here, as input is in row-major order
				output[i*cols+j]=s/((xmax-xmin)*(ymax-ymin)) # changed "rows" to "cols" here for the same reason
...
if __name__ == "__main__":
  ...
  # find /data/meme_images -name "*.jpg" | head -100 | xargs python FINd.py 
  # This will compute the FINd hash for the first 100 .jpg files in the /data/meme_images directory and print the hash and filename.
 
  # "find" is a command-line utility for searching files in a directory hierarchy. 
  # The "-name" option allows you to specify a pattern to match filenames.
  # "xargs" is used to execute the FINd.py script for each file found.